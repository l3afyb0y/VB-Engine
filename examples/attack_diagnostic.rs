...
fn main() {
    ...
    // Summary table.
    eprintln!("{:<28} {:>8} {:>6} {:>10} {:>10} {:>10}",
        "Component", "Peak", "PkIdx", "RMS", "DC_mean", "1st_neg");
    eprintln!("{:-<78}", "");
    for r in &results {
        eprintln!("{:<28} {:>8.5} {:>6} {:>10.6} {:>10.6} {:>10?}",
            r.name, r.peak, r.peak_idx, r.rms, r.dc_mean, r.first_neg);
    }

    // Dump per-component first-256 samples as TSV on stdout.
    // Header.
    print!("sample");
    for r in &results {
        print!("\t{}", r.name);
    ...
    for r in &results {
        ...
        for w in 0..8 {
            let start = w * 64;
            let end = (start + 64).min(r.first_256.len());
            if start >= r.first_256.len() { break; }
            let slice = &r.first_256[start..end];
            ...
            eprintln!("    [{start:>4}..{end:>4}]  energy={energy:.6}  dc={dc:+.6}  pos={pos_count}/{}", slice.len());
        }
    }
}
