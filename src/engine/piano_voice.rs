...
impl PianoVoice {
    ...
    pub(super) fn start(
        ...
        self.hammer
            .start(note, velocity, sample_rate_hz, soft_pedal_amount);
        self.strings
            .start(note, velocity, sample_rate_hz, soft_pedal_amount, self.pan);
    }

    pub(super) fn note_off(&mut self, pedal_is_down: bool) {
        if !self.active {
            return;
        }
        ...
        if pedal_is_down {
            ...
            self.sustained = false;
            self.released = true;
            self.strings.note_off();
        }
    }

    pub(super) fn pedal_release(&mut self) {
    ...
    pub(super) fn render(&mut self, hammer_noise_gain: f32) -> VoiceLayers {
        if !self.active {
        ...
        let transient_shape =
            1.0 + (hammer_frame.impact_emphasis * (0.015 + (0.015 * hammer_presence)));
        ...
        let direct_impulse = hammer_frame.direct_impulse * (0.010 * hammer_presence);
        let mut mechanical_left = direct_impulse * (0.72 - (self.pan * 0.12));
        let mut mechanical_right = direct_impulse * (0.72 + (self.pan * 0.12));
        let attack_left = hammer_frame.attack_left * (0.16 * hammer_presence);
        let attack_right = hammer_frame.attack_right * (0.16 * hammer_presence);
